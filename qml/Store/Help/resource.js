...
function download(state, data) {
    A.ANotification.addProgress(L.Con.Notification.DownloadResource, data.callID)
...
function saveToStorage(state, data) {
    A.ANotification.addProgress(L.Con.Notification.SaveResourceToStorage, data.callID)
    A.AAppToast.showInfo(qsTr("Download started.")+"\n"+qsTr("Check notifications for progress."))
}
